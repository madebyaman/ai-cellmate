...
async function processCsvEnrichment(data: CsvEnrichmentJobData): Promise<CsvEnrichmentJobResult> {
  const { csvUrl, enrichmentPrompt = 'Enhance the data in this CSV', userId } = data;

  console.log(`Starting CSV enrichment for URL: ${csvUrl}`);
  console.log(`Enrichment prompt: ${enrichmentPrompt}`);

  try {
    // Step 1: Fetch the CSV data
